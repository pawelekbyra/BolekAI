...
**Verified working (2026-07-15):** `npm run typecheck` clean, `npx vitest run` → 84/84 tests passing across 10 files, `next build` (web workspace) succeeds, `wrangler deploy --dry-run` compiles with correct bindings. MCP server mounted at `/mcp` (Bearer) and `/mcp/:secret` (URL path) with all tools converted to Zod schemas and callable from Claude Code / claude.ai. Daily visits report scheduled and logging to D1 with 36h KV dedup.
...
**Next: Faza 13+** — Two parallel tracks: (1) **Postgres migration** from D1 (infrastructure), multi-owner support, semantic memory retrieval; (2) **Remote computing** — Bolek gets headless Claude Code CLI on Oracle Cloud Free Tier VM (via Cloudflare Tunnel, `vm_claude_code` tool, with `--dangerously-skip-permissions` and machine-level isolation). See [`docs/BOLEK-COMPUTE-IDEAS.md`](docs/BOLEK-COMPUTE-IDEAS.md) for the latter (approved 2026-07-15, documenting the tradeoff against roadmap's safety-first principle). See "Future Work" in [`docs/NEXT-CODING-STEPS.md`](docs/NEXT-CODING-STEPS.md) for infrastructure track.
