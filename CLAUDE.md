...
- ✅ **2026-07-15 Session** — Multi-channel integration layer (MCP server `src/mcp.ts` + Vercel Web Analytics daily visits report `src/visits-report.ts`): Bolek's entire tool registry now exposed to Claude Code (Bearer auth `/mcp`) and claude.ai web UI (path-secret auth `/mcp/:secret`); Polutek traffic summary automated to 9:00 AM Warsaw time daily with DST-aware scheduling
- ✅ **2026-07-15/16 Session — Remote compute is live, not just planned.** Headless Claude Code now runs on an Oracle Cloud VM behind an HTTP wrapper (`bolek-agent.service`, systemd-managed) and is reachable from Bolek through `vm_claude_code` (`src/tools/vm-claude-code.ts`) — a high-risk, approval-gated tool exposed over a Cloudflare Quick Tunnel. Verified end-to-end through the real path (Telegram → policy engine → `/approve <id>` → VM → real file written and confirmed on disk, real cost reported: $0.0249 on Haiku). Also added: hourly Vercel runtime-error monitor (`src/log-monitor.ts`, KV-throttled to once/hour, alerts only on new/changed errors). **Known gap, not yet closed:** the tunnel is an unauthenticated, no-uptime-guarantee Cloudflare Quick Tunnel — fine for tonight's experiment, needs a named/authenticated tunnel before this is relied on for anything real.

**Verified working (2026-07-16):** `npm run typecheck` clean. MCP server mounted at `/mcp` (Bearer) and `/mcp/:secret` (URL path) with all tools converted to Zod schemas and callable from Claude Code / claude.ai. Daily visits report scheduled and logging to D1 with 36h KV dedup. `vm_claude_code` exercised live via the real Telegram → approval → VM path, not just a dry run.

**Corrected 2026-07-15 (two passes):** first pass found `/api/*` had no authentication at all despite Faza 1's DoD claiming otherwise, and `src/connectors/` was an unused duplicate of the real `src/tools/*.ts` integrations — both fixed. A second, independent pass found the "85+ regression tests" eval suite was tautological (compared hardcoded fixtures to themselves, called no production code), the voice interface was a hardcoded mock never wired into the webhook despite being marked ✅, and — found only *because* real evals were written — five approval-gated tools (`stripe_refund`, `email_send_reply`, `github_push_file`, `github_create_issue`, `vercel_redeploy`) had manifests requiring input fields that didn't match their actual parameters, meaning every real call to them would have been rejected before reaching the policy engine. All fixed. **Manual step required:** set the `BOLEK_API_KEY` secret (`wrangler secret put BOLEK_API_KEY`) and `NEXT_PUBLIC_BOLEK_API_KEY` (web build env) before deploying, or the dashboard/`/api/*` will 401.

**Corrected 2026-07-16 (found by actually using the feature, not just testing it):** the first real end-to-end test of an approval-gated tool since Faza 4 exposed two live bugs neither eval suite nor typecheck could have caught. (1) `BASE_SYSTEM_PROMPT` never told the orchestrator how to present a `requires_approval` result, so it improvised — describing an "Approve/Deny button" that doesn't exist (Telegram messages are sent with no `parse_mode`, so nothing renders as a clickable link), producing a convincing but false "done!" reply while every one of 15 approvals sat at `status: pending` in D1 with `approved_at: null`. Fixed by adding an explicit instruction to relay the approval ID and literal `/approve <id>`/`/deny <id>` command verbatim. (2) `handleActionConfirmation`'s regex required an exact `/approve <uuid>` match, so a command copy-pasted with the backtick code-formatting used to display it broke the match and silently fell through to the orchestrator instead of erroring. Fixed by stripping backticks/quotes/asterisks/whitespace before matching. See "Corrections (2026-07-16)" in [`docs/SYSTEM.md`](docs/SYSTEM.md) for full detail. **Lesson for future sessions:** an approval-gated tool that has never actually been approved and executed through the real user-facing path is unverified, regardless of what typecheck/evals/dry-run say.

**Next: Faza 13+** — Postgres migration from D1 (infrastructure), multi-owner support, semantic memory retrieval. See "Future Work" in [`docs/NEXT-CODING-STEPS.md`](docs/NEXT-CODING-STEPS.md). Remote compute (previously listed here as a future item) is done — see above and [`docs/BOLEK-COMPUTE-IDEAS.md`](docs/BOLEK-COMPUTE-IDEAS.md), which now documents the built system rather than the plan. Remaining hardening work on that track: replace the Quick Tunnel with a named/authenticated Cloudflare Tunnel, and decide whether `vm_claude_code` should stay permanently approval-gated or get a narrower auto-allow path for low-risk prompts.
